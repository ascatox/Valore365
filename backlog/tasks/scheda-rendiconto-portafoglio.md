...
### 3. Periodo di riferimento arbitrario
...
- Frontend: date picker per l'intervallo personalizzato accanto ai preset in `PerformanceKpiSummary.tsx:28-33`; gli hook in `hooks/queries.ts` oggi passano solo `startDate` benché il client API accetti già `endDate`.
