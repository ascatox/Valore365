...
const DASHBOARD_CHART_WINDOW_STORAGE_KEY = 'valore365.dashboard.chartWindow';

const formatPct = (value: number) => `${value >= 0 ? '+' : ''}${value.toFixed(2)}%`;
const getVariationColor = (value: number) => (value > 0 ? 'green' : value < 0 ? 'red' : 'gray');
const formatMoney = (value: number, currency = 'EUR', withSign = false) => {
...
export function DashboardPage() {
  ...
  const [chartWindow, setChartWindow] = useState<string>(() => {
    ...
    const stored = window.localStorage.getItem(DASHBOARD_CHART_WINDOW_STORAGE_KEY);
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem(DASHBOARD_CHART_WINDOW_STORAGE_KEY, chartWindow);
  }, [chartWindow]);
  ...
  const allocationByAssetId = useMemo(
    () => new Map(allocation.map((item) => [item.asset_id, item])),
    [allocation],
  );
  ...
  const assignedTargetNotional = useMemo(
    () => (portfolioTargetNotional != null ? (portfolioTargetNotional * totalAssignedWeight) / 100 : null),
    [portfolioTargetNotional, totalAssignedWeight],
  );
  const portfolioDeltaAmount =
    assignedTargetNotional != null && indexCardStats ? (assignedTargetNotional * indexCardStats.diffPct) / 100 : null;
  const bestAllocation = best ? allocationByAssetId.get(best.asset_id) ?? null : null;
  const worstAllocation = worst ? allocationByAssetId.get(worst.asset_id) ?? null : null;
  const bestTargetValue =
    portfolioTargetNotional != null && bestAllocation ? (portfolioTargetNotional * bestAllocation.weight_pct) / 100 : null;
  const worstTargetValue =
    portfolioTargetNotional != null && worstAllocation ? (portfolioTargetNotional * worstAllocation.weight_pct) / 100 : null;
  ...
  const kpiData = [
    {
      ...
      moneyLine:
        assignedTargetNotional != null && portfolioDeltaAmount != null && selectedPortfolio
          ? `Su ${formatMoney(assignedTargetNotional, selectedPortfolio.base_currency)}: ${formatMoney(
              portfolioDeltaAmount,
    ...
    },
    {
      ...
      moneyLine:
        bestTargetValue != null && best && selectedPortfolio
          ? `Target ${formatMoney(bestTargetValue, selectedPortfolio.base_currency)} • ${formatMoney(
              (bestTargetValue * best.return_pct) / 100,
              selectedPortfolio.base_currency,
              true,
            )}`
          : null,
    ...
    },
    {
      ...
      moneyLine:
        worstTargetValue != null && worst && selectedPortfolio
          ? `Target ${formatMoney(worstTargetValue, selectedPortfolio.base_currency)} • ${formatMoney(
              (worstTargetValue * worst.return_pct) / 100,
              selectedPortfolio.base_currency,
              true,
            )}`
          : null,
  ...
  const stats = kpiData.map((stat) => {
    ...
    return (
      <Paper withBorder p="md" radius="md" key={stat.label} shadow="xs">
        ...
        {'moneyLine' in stat && stat.moneyLine && (
          <Text size="xs" c={diffColor} mt="4">
            {stat.moneyLine}
          </Text>
        )}
      </Paper>
  ...
  const mvpKpiData = [
    ...
    },
    {
      ...
      value: String(portfolioPositions.length),
    },
    {
      label: 'Cash',
    ...
    <div style={{ padding: '20px' }}>
      ...
      <Card withBorder radius="md" p="md" mb="lg" shadow="sm">
        <Group justify="space-between" mb="sm">
          <Group gap="xs">
            <Title order={4}>Portfolio Tracker (MVP)</Title>
            <Badge variant="light">Dati reali da transazioni</Badge>
          </Group>
          <Button variant={showAdvanced ? 'filled' : 'default'} size="xs" onClick={() => setShowAdvanced((v) => !v)}>
            {showAdvanced ? 'Nascondi analisi avanzata' : 'Mostra analisi avanzata'}
          </Button>
        </Group>
        <Text size="sm" c="dimmed" mb="sm">
          Vista principale per uso quotidiano: valore, P/L, posizioni e allocazione reale.
        </Text>
        <SimpleGrid cols={{ base: 2, sm: 3, md: 5 }} spacing="sm" mb="md">
          {mvpKpiData.map((item) => (
            <Paper key={item.label} withBorder p="sm" radius="md">
              <Text size="xs" c="dimmed">{item.label}</Text>
              <Text fw={700} c={item.color ?? undefined}>{item.value}</Text>
            </Paper>
          ))}
        </SimpleGrid>
        ...
        <Card withBorder radius="md" p="md" mb="md">
          <Group justify="space-between" mb="xs">
            <Text fw={600} size="sm">Andamento Valore Portafoglio (reale)</Text>
            {mvpTimeseriesStats && (
              <Group gap="xs">
                <Badge variant="light" color="blue">
                  {formatMoney(mvpTimeseriesStats.last, mvpCurrency)}
                </Badge>
                <Badge variant="light" color={getVariationColor(mvpTimeseriesStats.pct)}>
                  {formatPct(mvpTimeseriesStats.pct)}
                </Badge>
              </Group>
            )}
          </Group>
          <Text size="xs" c="dimmed" mb="sm">
            Ultimi 90 giorni (o meno) calcolati da transazioni + storico prezzi.
          </Text>
          <div style={{ height: 220 }}>
            {mvpTimeseriesData.length === 0 ? (
              <Group h="100%" justify="center">
                <Text c="dimmed" size="sm">Nessun dato storico reale disponibile</Text>
              </Group>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={mvpTimeseriesData}>
                  <defs>
                    <linearGradient id="mvpTimeseriesGradient" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area
                    ...
                    strokeWidth={2.5}
                    fillOpacity={1}
                    fill="url(#mvpTimeseriesGradient)"
                  />
                </AreaChart>
              </ResponsiveContainer>
            )}
        ...
        <Grid gutter="md">
          <Grid.Col span={{ base: 12, lg: 8 }}>
            <Text fw={600} size="sm" mb="xs">Posizioni correnti</Text>
            <Table striped highlightOnHover>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>Asset</Table.Th>
                  <Table.Th style={{ textAlign: 'right' }}>Qta</Table.Th>
                  <Table.Th style={{ textAlign: 'right' }}>PMC</Table.Th>
                  <Table.Th style={{ textAlign: 'right' }}>Prezzo</Table.Th>
                  <Table.Th style={{ textAlign: 'right' }}>Valore</Table.Th>
                  <Table.Th style={{ textAlign: 'right' }}>P/L</Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {portfolioPositions.length ? (
                  portfolioPositions.slice(0, 10).map((p) => (
                    <Table.Tr key={p.asset_id}>
                      <Table.Td>
                        <Text fw={500}>{p.symbol}</Text>
                        <Text size="xs" c="dimmed">{p.name}</Text>
                      </Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>{p.quantity}</Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>{formatMoney(p.avg_cost, mvpCurrency)}</Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>{formatMoney(p.market_price, mvpCurrency)}</Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>{formatMoney(p.market_value, mvpCurrency)}</Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>
                        <Text c={getVariationColor(p.unrealized_pl)} fw={500}>
                          {formatMoney(p.unrealized_pl, mvpCurrency, true)} ({formatPct(p.unrealized_pl_pct)})
                        </Text>
                      </Table.Td>
                    </Table.Tr>
                  ))
          ...
          <Grid.Col span={{ base: 12, lg: 4 }}>
            <Text fw={600} size="sm" mb="xs">Allocazione (reale)</Text>
            <Table striped highlightOnHover>
              <Table.Thead>
                <Table.Tr>
                  <Table.Th>Asset</Table.Th>
              ...
              <Table.Tbody>
                {portfolioAllocation.length ? (
                  portfolioAllocation.slice(0, 8).map((item) => (
                  ...
                  <Table.Tr>
                    <Table.Td colSpan={2}>
                      <Text c="dimmed" ta="center" size="sm">Nessuna allocazione disponibile</Text>
                    </Table.Td>
                  </Table.Tr>
                )}
              </Table.Tbody>
                  ...
                  {chartWindow === '1' ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData} onClick={handleDailyChartClick}>
                        ...
                        <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#868e96', fontSize: 12 }} />
                        <YAxis hide domain={['auto', 'auto']} />
                        <Tooltip content={renderIndexTooltip('Data')} />
